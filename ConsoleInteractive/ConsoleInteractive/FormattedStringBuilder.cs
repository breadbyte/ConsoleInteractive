...
public class FormattedStringBuilder {
    ...
    public FormattedStringBuilder AppendMarkup(string text) {
        ...
        foreach (var newlineSplit in split) {
            ...
            if (idx > 0) {
                strings.Add(new StringData(newlineSplit[..idx]));
            }
            ...
            for (var i = 0; i < matches.Count; i++) {
                ...
                if (match.Groups[1].Value == "§k") {
                    ...
                    strings.Add(new StringData(new string('\u2588', match.Groups[2].Length), false, null, currentColor,
                ...
                if (i == matches.Count - 1) {
                    strings.Add(new StringData(match.Groups[2].Value, true, null, currentColor, currentFormattingType));
                }
                else {
            ...
            foreach (Match match in matches) {
                ...
                if (colorValue.Length > 1) {
                    ...
                    if (colorValue.StartsWith("38")) {
                        ...
                        currentColor = new(Convert.ToByte(splitted[2]), Convert.ToByte(splitted[3]),
                            Convert.ToByte(splitted[4]));
                    }

                    // 48 is the code for the Background Color.
                    else if (colorValue.StartsWith("48")) {
                        var splitted = colorValue.Split(';');
                        currentbgColor = new(Convert.ToByte(splitted[2]), Convert.ToByte(splitted[3]),
                            Convert.ToByte(splitted[4]));
                    }
                }
                else {
